 ...
 * deployment unit, on the Flyway side of the sticky `migrations`
 * dial. Emits `migrations/`: plain-SQL scripts (the shared
 * `migrations-sql` tree seeds `sql/V1__create_greeting.sql` in the
 * chosen engine's dialect) plus a `Dockerfile` that bakes them into
 * the official Flyway image, configured purely through `FLYWAY_*`
 * env vars. The runner is deployed and run against the database
 * **before** the service — never from inside it — so a bad migration
 * blocks the rollout instead of the fleet, and schema-altering
 * credentials never ship in the service image.
 *
 ...
 *
 * When the dial picks Liquibase this adapter contributes nothing —
 * `persistence/liquibase-migrations` covers the dimension instead.
 */
...
/** The engine-dialect SQL both migrations adapters seed `sql/` from. */
export const MIGRATIONS_SQL_TEMPLATE_ID = 'composition/persistence/migrations-sql/templates';
 ...
 */
export function migrationsServiceBlock(database: string, engine: SqlEngineSpec): string {
...
export const flywayMigrationsAdapter: Adapter = {
  ...
  async contribute(ctx) {
    if (migrationsTool(ctx.manifest) !== 'flyway') return {};
      ...
      patches: [
        {
          ...
          apply: eolAware((existing) => {
            if (existing.includes(SERVICE_MARKER)) {
              if (existing.includes('LIQUIBASE_COMMAND_URL')) {
                throw new Error(
                  `${FLYWAY_MIGRATIONS_ID}: dev/compose.yaml already carries the Liquibase migrations one-shot — remove that service (and migrations/) before switching the sticky 'migrations' answer to 'flyway', or keep Liquibase`,
                );
              }
              return existing;
            }
            return addComposeService(existing, migrationsServiceBlock(database, engine));
          }),
        },
        {
          target: 'README.md',
