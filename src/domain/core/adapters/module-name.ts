...
/**
 * Names rejected regardless of shape, and why each is here.
 *
 * Three groups, kept in one list because the user does not care which
 * group their word fell into:
 *
 * - **Keyword-ish** — `main`, `internal`, `test`, `core`, `crate`,
 *   `super`, `self`, `std`, `type`, `package`, `module`, `import`,
 *   `class`, `object`, `fun`, `var`, `val`, `func`, `interface`,
 *   `enum`, `record`, `impl`, `trait`, `use`, `mod`, `pub`, `static`,
 *   `final`, `new`, `default`: reserved in at least one of Go, Rust,
 *   Java or Kotlin, so the emitted `package ordering` line would be a
 *   syntax error. `internal` is the sharpest of these — it is not a Go
 *   keyword but a directory with compiler-enforced meaning, and a
 *   context named `internal` would make its own contents unreachable.
 * - **Structural** — `modules`, `platform`, `application`, `domain`,
 *   `contract`, `infra`, `service`, `kernel`, `src`, `build`,
 *   `target`, `dist`, `node`: directories the layouts already own. A
 *   context named `platform` would land its crates beside the shared
 *   kernel and read as though it were one.
 * - **Ecosystem** — `node_modules` is unspellable under the pattern
 *   anyway; `assets`, `docs` and `tests` are not reserved and are
 *   deliberately *absent*, because nothing breaks if a domain really
 *   is called that.
 */
