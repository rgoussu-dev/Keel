/**
 * Grid cell: **Micronaut CLI · Kotlin · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-kotlin-maven-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-kotlin-maven-cache-'),
  );
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Micronaut CLI, Kotlin, Maven)', () => {
  it(
