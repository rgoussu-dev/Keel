/**
 * Grid cell: **Quarkus REST · Kotlin · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-rest-kotlin-maven-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-quarkus-rest-kotlin-maven-cache-'),
  );
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Quarkus REST, Kotlin, Maven)', () => {
  it(
