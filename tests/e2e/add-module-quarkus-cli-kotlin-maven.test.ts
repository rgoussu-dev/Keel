/**
 * Grid cell: **Quarkus CLI · Kotlin · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-kotlin-maven-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-kotlin-maven-cache-'),
  );
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Quarkus CLI, Kotlin, Maven)', () => {
  it(
