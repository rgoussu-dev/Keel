/**
 * Grid cell: **Spring CLI + REST · Kotlin · Maven**, `modulith` layout —
...
beforeAll(async () => {
  mavenRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-mod-spk-mav-dep-'));
});
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-mod-spk-mav-'));
});
...
describe.skipIf(skipJvmMavenE2E)(
  'walking-skeleton modulith combo e2e (Spring CLI + REST, Kotlin, Maven)',
  () => {
    it(
      'builds one hexagon with both entrypoints, greets from the CLI jar and serves /greet',
      () =>
        runJvmComboE2E(
          { stack: 'spring-cli-rest-kotlin', moduleLayout: 'modulith', buildSystem: 'maven' },
          cwd,
          mavenRepo,
        ),
      E2E_TIMEOUT_MS,
