/**
 * Grid cell: **Spring CLI · Kotlin · Maven**, three bounded contexts
 ...
 * build file the new dependencies anchor in, and on Spring it also moves `@ComponentScan`
 * between `Main` and `Application`.
 *
...
import { E2E_TIMEOUT_MS, skipJvmMavenE2E, type JvmProjectSpec } from '../support/jvm-e2e.js';
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-cli-kotlin-maven-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-cli-kotlin-maven-cache-'));
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Spring CLI, Kotlin, Maven)', () => {
  it(
