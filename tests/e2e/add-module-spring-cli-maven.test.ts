/**
 * Grid cell: **Spring CLI · Java · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-cli-maven-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-cli-maven-cache-'));
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Spring CLI, Java, Maven)', () => {
  it(
