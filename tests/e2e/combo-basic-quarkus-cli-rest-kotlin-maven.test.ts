/**
 * Grid cell: **Quarkus CLI + REST · Kotlin · Maven**, `basic` layout —
...
beforeAll(async () => {
  mavenRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-bas-quk-mav-dep-'));
});
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-bas-quk-mav-'));
});
...
describe.skipIf(skipJvmMavenE2E)(
  'walking-skeleton basic combo e2e (Quarkus CLI + REST, Kotlin, Maven)',
  () => {
    it(
      'builds one hexagon with both entrypoints, greets from the CLI jar and serves /greet',
      () =>
        runJvmComboE2E(
          { stack: 'quarkus-cli-rest-kotlin', moduleLayout: 'basic', buildSystem: 'maven' },
          cwd,
          mavenRepo,
        ),
      E2E_TIMEOUT_MS,
