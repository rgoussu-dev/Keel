/**
 * Grid cell: **Spring CLI · Java · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-cli-gradle-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-cli-gradle-cache-'));
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Spring CLI, Java, Gradle)', () => {
  it(
