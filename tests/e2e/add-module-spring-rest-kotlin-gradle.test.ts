/**
 * Grid cell: **Spring REST · Kotlin · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-rest-kotlin-gradle-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-spring-rest-kotlin-gradle-cache-'),
  );
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Spring REST, Kotlin, Gradle)', () => {
  it(
