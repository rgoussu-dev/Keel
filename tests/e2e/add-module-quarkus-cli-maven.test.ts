/**
 * Grid cell: **Quarkus CLI · Java · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-maven-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-maven-cache-'));
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Quarkus CLI, Java, Maven)', () => {
  it(
