/**
 * Grid cell: **Micronaut CLI · Java · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-maven-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-maven-cache-'));
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Micronaut CLI, Java, Maven)', () => {
  it(
