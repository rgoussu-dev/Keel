/**
 * Grid cell: **Micronaut REST · Kotlin · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-kotlin-gradle-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-kotlin-gradle-cache-'),
  );
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Micronaut REST, Kotlin, Gradle)', () => {
  it(
