/**
 * Grid cell: **Micronaut CLI + REST · Java · Maven**, `modulith` layout —
...
beforeAll(async () => {
  mavenRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-mod-mij-mav-dep-'));
});
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-mod-mij-mav-'));
});
...
describe.skipIf(skipJvmMavenE2E)(
  'walking-skeleton modulith combo e2e (Micronaut CLI + REST, Java, Maven)',
  () => {
    it(
      'builds one hexagon with both entrypoints, greets from the CLI jar and serves /greet',
      () =>
        runJvmComboE2E(
          { stack: 'micronaut-cli-rest', moduleLayout: 'modulith', buildSystem: 'maven' },
          cwd,
          mavenRepo,
        ),
      E2E_TIMEOUT_MS,
