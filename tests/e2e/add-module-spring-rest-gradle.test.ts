/**
 * Grid cell: **Spring REST · Java · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-rest-gradle-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-rest-gradle-cache-'));
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Spring REST, Java, Gradle)', () => {
  it(
