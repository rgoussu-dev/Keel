/**
 * Grid cell: **Micronaut REST · Java · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-gradle-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-gradle-cache-'));
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Micronaut REST, Java, Gradle)', () => {
  it(
