/**
 * Grid cell: **Quarkus CLI · Kotlin · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-kotlin-gradle-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-kotlin-gradle-cache-'),
  );
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Quarkus CLI, Kotlin, Gradle)', () => {
  it(
