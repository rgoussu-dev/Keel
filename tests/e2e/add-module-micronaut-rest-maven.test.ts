/**
 * Grid cell: **Micronaut REST · Java · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-maven-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-maven-cache-'));
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Micronaut REST, Java, Maven)', () => {
  it(
