/**
 * Grid cell: **TypeScript CLI + HTTP · npm**, `modulith` layout —
 ...
 *
 * On npm rather than pnpm, and the two are not duplicates of each
 * other: npm's hoisting hides a missing dependency declaration that
 * pnpm's isolated store refuses, so a package borrowing a type
 * package from a sibling passes here and fails next door.
 *
...
beforeEach(async () => {
  cwd = await mkTempDir('keel-e2e-combo-mod-ts-npm-');
});
