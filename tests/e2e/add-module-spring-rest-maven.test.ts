/**
 * Grid cell: **Spring REST · Java · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-rest-maven-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-spring-rest-maven-cache-'));
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Spring REST, Java, Maven)', () => {
  it(
