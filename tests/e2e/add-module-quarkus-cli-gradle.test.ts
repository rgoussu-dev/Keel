/**
 * Grid cell: **Quarkus CLI · Java · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-gradle-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-quarkus-cli-gradle-cache-'));
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Quarkus CLI, Java, Gradle)', () => {
  it(
