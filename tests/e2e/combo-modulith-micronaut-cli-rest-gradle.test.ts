/**
 * Grid cell: **Micronaut CLI + REST · Java · Gradle**, `modulith` layout —
...
beforeAll(async () => {
  gradleUserHome = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-mod-mij-gra-dep-'));
});
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-combo-mod-mij-gra-'));
});
...
describe.skipIf(skipJvmComboE2E)(
  'walking-skeleton modulith combo e2e (Micronaut CLI + REST, Java, Gradle)',
  () => {
    it(
      'builds one hexagon with both entrypoints, greets from the CLI jar and serves /greet',
      () =>
        runJvmComboE2E(
          { stack: 'micronaut-cli-rest', moduleLayout: 'modulith', buildSystem: 'gradle' },
          cwd,
          gradleUserHome,
        ),
      E2E_TIMEOUT_MS,
