/**
 * Grid cell: **Micronaut CLI · Java · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-gradle-'));
  depCache = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-gradle-cache-'));
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Micronaut CLI, Java, Gradle)', () => {
  it(
