/**
 * Grid cell: **Micronaut REST · Kotlin · Maven**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-kotlin-maven-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-micronaut-rest-kotlin-maven-cache-'),
  );
});
...
describe.skipIf(skipJvmMavenE2E)('add-module e2e (Micronaut REST, Kotlin, Maven)', () => {
  it(
