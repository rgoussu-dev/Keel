/**
 * Grid cell: **Micronaut CLI · Kotlin · Gradle**, three bounded contexts
...
beforeEach(async () => {
  cwd = await fs.mkdtemp(path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-kotlin-gradle-'));
  depCache = await fs.mkdtemp(
    path.join(os.tmpdir(), 'keel-e2e-am-micronaut-cli-kotlin-gradle-cache-'),
  );
});
...
describe.skipIf(skipJvmE2E)('add-module e2e (Micronaut CLI, Kotlin, Gradle)', () => {
  it(
