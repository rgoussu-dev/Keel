...
holds too; at a product root, a ● is what `keel new --with` on the
product sets aside as already there, which it holds as well.
...
- **The product root takes what is the product's own.** It has `vcs`,
  and `dev-env` adds a root `dev/compose.yaml`. What its services have
  and none of them could take again — the harness, code style, the
  images the root builds, `observability` in the one service that can
  carry it — is there already: `keel add` of it at the root is an Ok
  that adds nothing, naming the services, as `keel new --with` of it
  on the product sets it aside. `keel add` refuses anything else
  there, and where a service can take it the refusal names that
  service; at `keel new`, a bare `--with` naming one goes to the one
