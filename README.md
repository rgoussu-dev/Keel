...
| [`toolchain`](docs/verticals/toolchain.md)               | records the toolchain the project needs (JDK, build system, Node, …) in the manifest, from keel's pins; `keel toolchain install` provisions it through the manager you pick — mise, asdf, nvm (+ corepack) | every stack (opt-in)              |
